...
> [!TIP]
> 体系的なステップバイステップの解説書（チュートリアル）をお探しの場合は、[Shark プログラミング入門・実践解説書](tutorial.md) をご参照ください。
...
| 必要になったら | ジェネリクス 無名関数（ラムダ式） `task` `parallel` `std.ui` | 7、8、14、15 |
...
関数に名前を付けず、関数の実引数や変数代入の位置に直接記述できます。構文は通常の関数定義と同様で、関数名のみを省略します。
...
関数の値は「**どの関数か**」を指す値であり、外側のローカル変数をキャプチャしません。そのため
クロージャオブジェクト等の余計なメモリ確保が発生せず、値コピーのセマンティクスとも整合します。
必要な値は引数として受け渡し、共有したい状態はトップレベルの `var` に定義します。
...
## 15. GUI・グラフィック描画（std.ui）

`std.ui` は2つのレイヤーで構成されています。**低レベル層**はピクセルバッファ（サーフェス）とマウス・キーボードイベントを直接制御し、**高レベル層**はウィジェットツリーを宣言的に構築して返します。
両者を併用することも可能です。

`shark` コマンドは、ネイティブウィンドウを直接開きます（外部 GUI ツールキットには依存しません）。
ウィンドウ表示に対応していない環境（SSH 接続時など）では、オフスクリーンバッファに描画されます。

### 低レベル描画 API — プリミティブ描画とイベントループ
...
### 高レベル宣言的 UI — Widget ツリーの構築

ウィジェットを構築して **Widget ツリーとして返す** と、その構成に従って描画されます。
イベントループや再描画処理は `ui.run()` が一括管理し、**イベントハンドラはウィジェットに直接設定** します。
...
イベントを一箇所に集約して処理したいときは、関数の代わりに **ID（識別名）** を渡して `update()` で分岐します。
...
操作時に変数が直接更新されるため、ID や `update()`、`ui.value()` を介した値取得は不要です。
...
`ui.show()` が返すのは、操作されたウィジェットの **ID**（指定した `id` 文字列）です。
...
スクロール機能を持つウィジェット（`ui.listbox`・`ui.textarea`・`ui.combo` のドロップダウン）は、
**カーソルをホバーしてマウスホイールを回すことでもスクロール可能**です。クリックする必要はありません。
スクロールは行単位ではなく**ピクセル単位でスムーズに補間**されます。
カスタムスクロールを実装する場合は `ui.wheel()`（下方向が正）と `ui.wheel_x()`（右方向が正）で取得します。
ウィジェットが消費したスクロール量は 0 にクリアされるため、二重に動作することはありません。
...
字の大きさは `.font(px)` で変えられます。**中に入れた部品にも効きます。**
部品の余白も、印の大きさも、並べたときの間も、すべて字の大きさに対する比で
決まっているので、釣り合ったまま一緒に大きくなります。

```shark
ui.col([
  ui.label("設定").font(24),           // 見出しだけ大きく
  ui.label("あとから変えられます").font(10),
  ui.button("閉じる", "close"),        // ここは元の大きさのまま
]),
```

**マウスが無くても操れます。**`tab` で次の部品へ、`shift+tab` で前へ焦点が移り、
`space` か `enter` で押せます。つまみとタブの見出しは左右のキー、一覧と数の欄は
上下のキーで動きます。焦点は**置いた順**に回り、止めてある部品（`.disabled`）は
飛ばします。画面はゲーム機が持っている、という前提の言語なので、
マウスを当てにしない操り方をひととおり持たせてあります。
...
ui.col([
  ...
  ui.col([
    ui.field(ref name),
    ui.button("進む", "go"),
  ]).disabled(!ok),                    // ok になるまで、中はぜんぶ止まります
]),
```
...
細かさが**半端な数**の画面もあります（Windows の 125% / 150% 表示、ブラウザの拡大、
携帯の画面）。`ui.scale()` はそれを整数に丸めた数なので、細かさ 1.5 の画面では
上の書き方は 2 倍の面になり、見た目が 1.33 倍に大きくなります。
丸めない数がほしいときは `ui.pixel_ratio()` を使います。

```shark
var r = ui.pixel_ratio();                                     // 1.0 / 1.25 / 1.5 / 2.0 …
ui.open("さめ", int(420.0 * r + 0.5), int(300.0 * r + 0.5));   // 見た目はいつも 420×300
_ = ui.font(int(12.0 * r + 0.5));
```

`ui.run()` はこちらで大きさを決めているので、何もしなくてもそうなります。

部品の寸法は字の大きさから決まるので、`ui.font()` を変えれば全体の釣り合いが付いてきます。

### ヘッドレス環境での動作（オフスクリーン描画）

画面出力を持たない環境や `SHARK_UI=off` 設定時は、描画先が**オフスクリーンバッファ**になります。
描画 API の使い方は全く同一であり、描画結果は `ui.get()` で取得できるため、
**GUI アプリケーションでも自動テストを記述可能** です。
...
内蔵フォントは ASCII のみです。日本語等のアウトラインフォントを描画するには、FreeType をリンクしてビルドした上で `ui.font()` を呼び出します（ビルド手順は [README](../README.md) の「日本語フォント描画」参照）。
...
フォントを明示指定しない場合は内蔵フォントが維持されます。内蔵フォントであれば全プラットフォームで同一の表示結果となりますが、OS のフォントを読み込んだ場合は `ui.text_width()` の戻り値も当該フォントのメトリクスに従います。

### 日本語入力（IME）の対応

`ui.field` および `ui.textarea` は日本語入力に対応しています。かな漢字変換処理は**プラットフォーム側に委譲** されます。

| プラットフォーム | 動作 |
|---|---|
| macOS ウィンドウ | OS ネイティブの IME が動作。未確定文字列は下線表示 |
| Windows ウィンドウ | OS ネイティブの IME（IMM32）が動作。候補ウィンドウを適切に配置 |
| ブラウザ環境 | ブラウザ標準の IME が動作 |
| その他の環境 | 入力された文字がそのまま反映されます |
...
macOS ウィンドウでは Cmd-C / Cmd-V / Cmd-X / Cmd-A のショートカットも動作します。
置き場は `ui.clipboard()` と `ui.set_clipboard()` でプログラムからも触れます。

自分でメニューを出したいときは `ui.menu(x, y, 並べるもの)` と `ui.menu_pick()` です。
どちらのメニューも、押して離してからもう一度押しても、**右で押したまま動かして離しても**選べます。

自分で入力欄を描くときは `ui.input()` と `ui.marked()` を使います。

### 気をつけること

- 内蔵フォントは ASCII のみです。FreeType 未リンク時は日本語文字が □ と表示されます（ウィジェットのラベルも同様）
- キーリリースイベントに対応していないプラットフォームが存在します。キー押し下げによる連続移動を行う場合は、`ui.pressed()` で状態を管理することで全プラットフォームで一貫した動作になります
...
[examples/widgets.shk](../examples/widgets.shk)（全 UI ウィジェットの総合デモ）、
...
改修が必要なのはプラットフォーム移植層（ターゲット環境向けの実装への差し替え）と、
ホスト関数の登録（ゲーム固有 API の追加）の2箇所のみで、それ以外はそのまま使えます。
構文と型の規則は変えません。
...
| 区分 | 内容 |
|---|---|
| 実装しながら決める | 日本語フォント描画と IME 入力、ウィジェット種類の拡張、JIT コンパイルの閾値、Unicode テーブルサイズ、C++ 標準規格 |
