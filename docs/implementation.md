...
`--jit off` で実行時コンパイルを切り、仮想マシンだけで動かす（下の[実行時コンパイル](#実行時コンパイルcorejitcpp)）。
...
6. 機械語を置ける機種なら `PlatformExec` も埋める。埋めなければ実行時コンパイルをしない
   （下の[実行時コンパイル](#実行時コンパイルcorejitcpp)）
7. ホストの最初に `platform_set(platform_自分の機種());` を呼ぶ
...
### 実行時コンパイル（core/jit.cpp）

何度も通る関数を、その場で機械語にして差し替える
（[spec/runtime/execution.md](../spec/runtime/execution.md)）。**任意機能**で、
作り方を知らない機種や、実行できるメモリを借りられない機種では、
何もせず今までどおり仮想マシンで動く。

| | |
|---|---|
| いつ作るか | 呼ばれた回数とループの回転数を数え、**40** を超えた関数だけ |
| 何を作るか | 関数まるごと。バイトコードの命令1つが機械語のひとかたまりになる |
| 分からない命令 | **その1つだけ仮想マシンに戻す**（`VM::step(1)`）。続きは機械語に戻る |
| 置き場 | 移植層の `PlatformExec`（`core/platform/platform.h`） |
| 機種 | ARM64（`core/jit_arm64.inc`）と x86-64（`core/jit_x64.inc`）。ほかの機種は仮想マシンのまま |
| 切り方 | `Config::jit`（`shark` なら `--jit off`） |

値の置き場（`TaskState::stack`）と枠は仮想マシンとまったく同じものを使う。
積み方も変えないので、**どの命令の切れ目でも仮想マシンに戻れる**。これが
「どちらで実行しても結果は同じ」の土台で、次の場面ではその場で戻している。

- 作り方を知らない命令（代入先の組み立て、文字列づくり、`task`、`parallel` など）
- **あふれ・0 で割る・範囲の外**など、つまずきうるところ。
  機械語には速い道だけを置き、外れたら仮想マシンに戻す。
  そのため panic の文面も、止まる場所も、経路もそのまま同じになる
- 刻み（budget）が尽きたとき。ループの入口で見て、ホストに返る
- 待つ関数（`N_Wait`）とタスクの切り替え

機械語にしてあるのは、よく通るところだけ。

- 定数・局所変数・グローバル・クラスの中身の読み書き（参照数の増減もその場で）
- `int` と `float` の四則、ビット演算、ずらし、比べもの（`int` どうしは直に）
- 分岐と、`range` を回す `for`
- 配列の添字（`xs[i]`）
- 関数の呼び出しと戻り、処理系が持つ関数（`xs.push(x)` など）の呼び出し

作った機械語はバイトコードの 14〜18 倍の大きさになる（1関数あたり 2 KB ほど）。
しきい値があるのは、**この置き場を増やさない**ため。作る時間そのものは
1関数あたり数マイクロ秒で、測っても差が出ない。

**別の機種に移すとき**は `core/jit_arm64.inc` か `core/jit_x64.inc` にならって
1つ足し、`core/jit.cpp` の機種ごとの `#include` に加える。土台（どこで戻すか、
熱さの数え方、置き場の管理、手伝いの関数）は機種によらず `core/jit.cpp` にある。
機種ごとの `.inc` が受け持つのは「バイトコードの命令1つぶんの機械語を、
バイトの並びで書く」ことだけで、どの命令をどう扱うかは2つとも同じ形をしている。

x86-64 は呼び出しの決まりが2つある（Windows と、macOS・Linux の System V）。
違いは引数の置き場所と、呼ぶ側が空けておく枠だけなので、`arg_reg` / `arg_imm` と
前口上に閉じ込めてある。機械語を置く場所は、Windows は `VirtualAlloc`、
それ以外は `mmap`（`core/platform/desktop.cpp`）。
...
| | |
|---|---|
| ループ・配列・key-value・関数呼び出し | Python より速い |
| 文字列づくり | Python より少し遅い |
| 最適化した C | 4〜20 倍の差 |

[実行時コンパイル](#実行時コンパイルcorejitcpp)を入れる前と比べると、
同じ測りかたで次のようになった（`--jit off` が入れる前の速さ）。

| 内容 | 仮想マシンだけ | 機械語にして | |
|---|---|---|---|
| 整数のループ 1000万回 | 312 ms | **49 ms** | 6.4 倍 |
| 再帰呼び出し `fib(32)` | 340 ms | **134 ms** | 2.5 倍 |
| 可変長配列 100万件×5回 | 234 ms | **133 ms** | 1.8 倍 |
| key-value 50万件 | 51 ms | **34 ms** | 1.5 倍 |
| 書式付きの文字列 100万個 | 175 ms | **153 ms** | 1.1 倍 |

文字列づくりが伸びないのは、時間のほとんどが文字列そのものを作るところ
（確保と書き出し）にあり、命令を回す時間ではないため。
仮想マシンだけで動かしたときの速さは、実行時コンパイルを入れる前と変わらない
（命令ごとに増えたのは「機械語があるか」の比べもの1つだけ）。
...
| # | 決めたこと |
|---|---|
| 2 | 埋め込んだ Unicode の表は仮名の並び順（83 文字ぶん）と表示幅の範囲だけ。数百バイトに収まっている |
| 3 | **C++17** に合わせた。例外と RTTI は使わない（`-fno-exceptions -fno-rtti`）。標準ライブラリは `<stdint.h>` `<new>` などの薄い範囲と、数の書き出しに使う `snprintf`、`std.math` の `<math.h>` だけ |
| 4 | ホスト関数に渡せるのは基本型とコレクションのまま |
| 5 | 実行の途中状態の保存は入れていない |

実行時コンパイルのしきい値も決まり、一覧から外した。
**40**（呼ばれた回数＋ループの回転数）で、作る時間ではなく
機械語の置き場を増やさないための線 → [実行時コンパイル](#実行時コンパイルcorejitcpp)。
