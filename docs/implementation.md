...
| [移植する](#新しいプラットフォームへの移植) | 新規プラットフォームへの移植手順 |
| [中身の作り](#中身の作り) | コアの内部 |
...
`--memory <MB>` で使ってよいメモリの量を決める（既定は 256 MB、`0` で上限なし。下の[メモリの上限](#メモリの上限)）。
...
4. 画面描画を行う場合は `PlatformScreen` を実装する。未実装でも `std.ui` はオフスクリーンバッファとして動作可能
5. プラットフォーム側でフォント描画 API を提供する場合は `PlatformFont` を実装する（未実装時は内蔵 5×7 ASCII フォントのみ）
6. ホスト初期化時に `platform_set(platform_target());` を呼び出して登録する
...
`shark` CLI のデスクトップ移植層（`core/platform/desktop.cpp`）は、3種類の `PlatformScreen` バックエンドを
実装しており、実行環境に応じて自動選択します。いずれも利用できない場合はヘッドレスモードとなり、
オフスクリーンバッファへ描画されます。環境変数 `SHARK_UI=off` を指定すると、ウィンドウ生成を行わずオフスクリーン描画に固定できます。

| バックエンド | ソースファイル |
|---|---|
| macOS ウィンドウ | `core/platform/screen_mac.inc` |
| Windows ウィンドウ | `core/platform/screen_win.inc` |
| X11 ウィンドウ | `core/platform/screen_x11.inc` |

**ビルド時の外部ライブラリ依存はありません。** ウィンドウ生成に必要な API は実行時に動的ロードします
（`dlopen` / `LoadLibrary`）。X11 開発パッケージがインストールされていない環境でも、
問題なくコンパイルして実行可能です（ウィンドウが開かない場合はヘッドレスにフォールバック）。
...
- クリップボードは `CF_UNICODETEXT` を利用。**ウィンドウを開いていなくても利用可能**
...
- **HiDPI**：`canvas` の**内部ピクセル解像度**はキャンバスサイズと一致させ、CSS 上の表示サイズを
  `devicePixelRatio` で除算して決定する。ここでは**浮動小数点値**をそのまま使用する —
  ブラウザの DPR は 1.25 や 1.5 などの非整数値も一般的であり、整数に丸めると
  canvas のピクセルが物理ピクセルグリッドとずれてブラウザ側で再サンプリングされ、**表示が滲む**。
- スケーリング方式（`image-rendering`）は、物理ピクセルとの倍率が正確に整数のときのみ
  `pixelated` を適用（ドットバッファの鮮明さを維持）。表示領域に収まらず縮小表示する場合は
  `auto` を設定してバイリニア補間を行う（ニアレストネイバーでは縮小時にモアレやライン欠落が生じるため）。
...
`SEV_Wheel` の `x`, `y` は **1/100 行**（下と右が正。1段ぶん回すと 100）。
届く単位は機種でまちまちなので、**行に直すのは移植層**
（[spec/library/ui.md](../spec/library/ui.md)）。

| バックエンド | 入力イベント値 | 変換ロジック |
|---|---|---|
| macOS ウィンドウ | `scrollingDeltaX/Y`。トラックパッドはポイント単位、ホイールは行単位 | `hasPreciseScrollingDeltas` 時は 16 で除算。上方向が正のため符号反転 |
| Windows ウィンドウ | `WM_MOUSEWHEEL` / `WM_MOUSEHWHEEL`。120 の倍数 | 120 で除算して行単位に正規化（100 の倍数） |
| X11 ウィンドウ | Button 4〜7 の押下イベント | 4/5 が垂直、6/7 が水平。1イベントあたり 100（1行分） |
| ブラウザ環境 | `wheel` イベントの `deltaX/Y` | ピクセル単位時は 16 で除算。方向はそのまま保持 |

**行より細かく渡す**のが肝心なところ。行の数で渡すと、トラックパッドを
ゆっくり動かしたときに1行たまるまで何も動かず、動き出すと1行飛ぶ。
1画素に満たないぶんを持ち越すのはコア（`core/lib/ui.cpp` の `take_wheel_px`）で、
`ui.wheel()`（行の数で返す）も同じように持ち越す。
どちらも [tests/uicheck.cpp](../tests/uicheck.cpp) が見張っている。
...
- IME プロキシ（隠し入力ビュー）にフォーカスがある間は、プラットフォーム層から `SEV_Text` を発行しない（入力が二重に処理されるのを防ぐため）
...
- **Windows は IMM32** を動的にロードして使用する。macOS の `NSTextView` にあたる
  ネイティブ入力ビューが存在しないため、**入力バッファはプラットフォーム層が保持する**（`screen_win.inc` の `Input`）。
...
- **複数行テキスト入力（`ui.textarea`）**では、`text_input` の `multiline` を `true` に設定する。
  IME プロキシは改行文字を保持するが、**行関連のキー（Enter、上下矢印、Home、End）はネイティブ側で処理しない**（macOS ではプロキシビューへ渡さず、WebAssembly では `preventDefault` を呼び出し、
  Windows では `edit_key` でバイパスする）。折り返し幅を管理しているのはコア側のみであり、
  プロキシビューに任せると**ネイティブ側の折り返し計算によってカーソルが意図せず再移動してしまう**ためである。
  改行の挿入もコア側で制御し、`text_replace("\n")` としてプロキシビューへ反映する。
  単行入力欄（`ui.field` / `ui.input`）に改行が含まれた場合は、コア側で除去する
  （`core/lib/ui.cpp` の `flatten`）。

#### テキスト選択・クリップボード・コンテキストメニュー

- **IME 変換中のみ、プラットフォーム側の選択位置を参照しない。**IME プロキシが未確定文字列も含めて 1 つのバッファとして保持する環境（macOS の `NSTextView`、ブラウザの `<textarea>`）では、
  `text_selection` が返す位置に**変換中の未確定文字が含まれる**のに対し、
  `text_state` が返す確定文字列にはその文字が**含まれない**。両者の文字数基準が異なるため、
  そのまま同期すると変換文字数が増加するたびにカーソル位置がずれてしまう。
  そのため、コア側は変換中はカーソルを移動させず、**変換開始時の位置**に固定し、
  確定後に再計算する（`core/lib/ui.cpp` の `place_field` / `place_area`）。
  この仕様は [tests/imecheck.cpp](../tests/imecheck.cpp) で検証している
  （同様のインデックス計算を行うモック画面で入力欄を操作し、未確定文字列の描画位置をピクセル単位で検証）。
  クリックやフォーカス遷移の動作は [tests/uicheck.cpp](../tests/uicheck.cpp) で同様に検証する。
  いずれもモックスクリーンは [tests/fake_screen.inc](../tests/fake_screen.inc) に実装されている。
...
- クリップボードは macOS の `NSPasteboard`、Windows の `CF_UNICODETEXT` を使用。
  どちらも**ウィンドウを開いていなくても利用可能**なため、ヘッドレス実行時でも同一のクリップボード領域にアクセスできます
- macOS ではメニューバーを持たない場合、Cmd-C などの標準ショートカットが OS 側で自動ディスパッチされないため、
  ネイティブ入力ビュー（`NSTextView`）の `copy:` `paste:` `cut:` `selectAll:` `undo:` を直接呼び出す。
  Windows も同様にメニューバーを持たないため、Ctrl+A/C/V/X はプラットフォーム層で直接処理する。
...
- フォーカス状態は内部IDで管理されるため、ID はスロット番号から生成する（先頭が 0x01 の内部キー文字列）。
  ユーザーが明示的に付けた ID とは衝突せず、`ui.show()` の戻り値として返されることもない
...
- グリフキャッシュは (文字コード, サイズ) をキーとするオープンアドレス法のハッシュテーブル。満杯時は全消去して再構築
- **フォントフォールバック。**1本のフォントに世界中の字は入っていない。
  同梱の Noto Sans JP も、絵文字やハングルは持たない。そこで
  `FT_Get_Char_Index` が 0（その字を持っていない）なら、控えのフォントを探す
  - フォールバックフォントは**必要時に遅延ロード**。不要な文字が使われない限りロードしない
...
- **フォントデータおよびグリフキャッシュは `malloc` によりヒープ上に直接確保する。**日本語のフォントは数 MB あり、
...
| 使う側 | 書き方 | 既定 |
|---|---|---|
| 組み込み | `Config::memory_limit`（バイト。`0` は上限なし） | 256 MB |
| `shark` コマンド | `--memory <MB>`（`0` は上限なし） | 256 MB |
...
- シリアライズ対象は、バイトコード命令列・定数プール・行番号マップ・関数・クラス・グローバル変数・エントリ順。
  AST（抽象構文木）やソースコード文字列は含まない。
- 数値は可変長エンコーディング（LEB128 相当）で格納。参照インデックス（型・クラス・関数）は整数インデックスで表し、未定義は `-1`。
- 識別子名・モジュール名・ファイル名・文字列定数は**文字列プール（テーブル）に集約**し、本体からはインデックス参照する。
  同一文字列の重複を排除しファイルサイズを削減する。
- ヘッダーに**チェックサム**（後続データの 4 バイトハッシュ）を格納。
  破損や改ざんをロード前に高速に検知してエラーとする。
- 型定義は**依存先が先に来るトポロジカルソート順**で配置するため、前方から 1 パスで復元可能
  （`TypeTable` のファクトリメソッドを再利用し、同一構造の型は自動で単一化される）。
- ネイティブ組み込み関数は関数インデックスで参照。インデックスは登録順に依存するため、
  モジュール登録は `register_modules()` に一元化し、**API シグネチャのハッシュ値**をヘッダーに含めて照合する。
  不一致時はロードを拒否する。
...
| `shark repl`（対話） | 1行ずつ試したいため。入力ごとに1つのモジュール（名前は `main`）として足していき、前の入力の型検査とコード生成はやり直さない。定義し直した名前は、前のものの名前を変えて退ける（[spec/frontend.md](../spec/frontend.md)） |
