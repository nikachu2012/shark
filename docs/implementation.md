...
| 機種 | いまは ARM64 だけ（`core/jit_arm64.inc`）。ほかの機種は仮想マシンのまま |
...
作った機械語はバイトコードの 14〜17 倍の大きさになる（1関数あたり 2 KB ほど）。
しきい値があるのは、**この置き場を増やさない**ため。作る時間そのものは
1関数あたり数マイクロ秒で、測っても差が出ない。

**別の機種に移すとき**は `core/jit_arm64.inc` にならって1つ足し、
`core/jit.cpp` の機種ごとの `#include` に加える。土台（どこで戻すか、
