...
| 出し先 | 届くもの | どう直すか |
|---|---|---|
| 窓（macOS） | `scrollingDeltaX/Y`。トラックパッドは点、車輪は行 | `hasPreciseScrollingDeltas` なら 16 で割る。上が正で届くので裏返す |
| 窓（Windows） | `WM_MOUSEWHEEL` / `WM_MOUSEHWHEEL`。120 の倍数 | 120 で割って**行きっかり**にする（細かくは送れないので、100 の倍数になる） |
| 窓（X11） | ボタン 4〜7 の押下 | 4/5 が縦、6/7 が横。1回で 100（1行） |
| ブラウザ | `wheel` の `deltaX/Y`（`deltaMode` で単位が変わる） | 画素なら 16 で割る。向きはそのまま |
...
- **複数行の入力欄（`ui.textarea`）**では、`text_input` の `multiline` を `true` にする。
  受け皿は改行を落とさずに持ち、**行にまつわるキー（`enter`・上下・`home`・`end`）を
  自分では扱わない**（macOS は受け皿へ渡さない、ブラウザは `preventDefault`、
  Windows は `edit_key` が受けない）。折り返す幅を知っているのはコアの側だけで、
  受け皿に任せると**向こうの折り返し方でもう一度動いてしまう**ため。
  改行を入れるのもコアで、`text_replace("\n")` として受け皿に渡す。
  1行の入力欄（`ui.field` / `ui.input`）に改行が混ざったときは、コアが落とす
  （`core/lib/ui.cpp` の `flatten`）

#### 選ぶ・切り貼り・メニュー

- **変換中だけは、移植層の位置を見ない。**受け皿が変換中の字も1つの文字列として
  持つ機種（macOS の `NSTextView`、ブラウザの `<textarea>`）では、
  `text_selection` が返す位置に**変換中の字が数に入っている**のに、
  `text_state` が返す確定文字列にはその字が**入っていない**。物差しが違うので、
  そのまま合わせると変換中の字が増えるたびにカーソルが1文字ずつずれる。
  コアは変換のあいだカーソルを動かさず、**変換を始めたところ**に置いたままにして、
  確定してから数え直す（`core/lib/ui.cpp` の `place_field` / `place_area`）。
  この決まりは [tests/imecheck.cpp](../tests/imecheck.cpp) が見張っている
  （同じ数え方をする偽の出し先で入力欄を動かし、変換中の字が出る位置を絵で確かめる）。
  押した・合わせたときの動きは [tests/uicheck.cpp](../tests/uicheck.cpp) が同じ作りで見る。
  どちらも偽の出し先は [tests/fake_screen.inc](../tests/fake_screen.inc) にまとめてある
