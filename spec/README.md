...
| 応答性の担保 | 無限ループ等でホストゲームがフリーズしないよう、タイムスライス（ステップ）実行に対応 |
...
- `platform/` をターゲットプラットフォーム向けの実装に**差し替える**
- `host` にゲーム固有の操作（ホスト関数）を**追加する**

それ以外はそのまま使えることを目指す。改修箇所を局所化することで、
上流リポジトリの修正を取り込みやすくする。
...
| [runtime/bytecode.md](runtime/bytecode.md) | バイトコードの保存と、単一バイナリにする形 |
