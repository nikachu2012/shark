...
main のスコープ（count がある）
 └ ui.window(...) { ... }   ← ウィンドウが閉じるまで待機
```

`ui.window` はウィンドウが閉じるまで終了しないため、`count` は必ず生存している。
...
| 関数 | 説明 |
|---|---|
| `ui.window(title: string) { }` | ウィンドウ。閉じるまで待機 |
| `ui.column { }` | 縦に並べる |
| `ui.row { }` | 横に並べる |
| `ui.stack { }` | 重ねる |
...
| 関数 | 説明 |
|---|---|
| `ui.quit()` | ウィンドウを閉じて `ui.window` から脱出 |
| `ui.redraw()` | 明示的に描き直す |
...
- 文字描画にはフォントが必要。OS のシステムフォントを使用し、未検出時は内蔵フォントにフォールバックする
