...
出力は UTF-8 の文字列として返る。Windows のように、システムのコードページ（日本語環境では
CP932）で出力するプログラムと UTF-8 で出力するプログラムが混在する環境では、移植層が
**UTF-8 として妥当であればそのまま解釈し、不正バイト列であればシステムの文字コードとしてフォールバック解釈する**。
