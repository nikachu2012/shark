...
名前を定義せず、関数の実引数や変数代入の位置に直接関数を記述できる（無名関数 / 関数リテラル）。記述方法は通常の関数と同様で、関数名のみを省略する。
...
- 関数の値が**どの関数かを指すポインタ**として完結する。外側のローカル変数をキャプチャ（クロージャ化）しないため、
  値の受け渡しやコピー時に追加のメモリ確保が発生しない（[runtime/memory.md](runtime/memory.md)）
