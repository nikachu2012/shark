...
仕様策定にあたり保留とした検討項目の一覧。各仕様書の検討課題を集約したもの。
決定済みの項目は [下](#決定事項) に移動する。

机上設計で合意可能な仕様は一通り策定済み。残る検討項目は以下のカテゴリに分類される。

| # | 決めること | 区分 | 直すファイル |
|---|---|---|---|
| 1 | GUI まわりの残課題（FreeType 未リンク時の日本語描画、IME 入力） | 実装時 | library/ui.md |
...
| 無名関数のスコープ | レキシカル変数をキャプチャしない。参照可能なのは自身の引数・ローカル変数およびトップレベル定義のみ。関数ポインタとして軽量に扱い、値コピーの原則を維持する | [syntax.md](syntax.md) |
