...
| 2 | 埋め込む Unicode の表の大きさ | 実装時 | library/overview.md |
| 3 | C++ のどの規格に合わせるか | 実装時 | skeleton.md |
| 4 | ホストの関数にクラスを渡せるようにするか | 組み込み | runtime/embedding.md |
| 5 | 実行の途中状態を保存できるようにするか | 組み込み | runtime/embedding.md |
| 6 | 言語サーバとして常駐させるか | 別実装 | frontend.md |
| 7 | 整形の細かい規則 | 別実装 | frontend.md |
...
### 4. ホストの関数にクラスを渡せるようにするか

いまは基本型とコレクションだけ。ゲーム側の「ロボット」のような値を
そのまま渡したくなったら考える。

### 5. 実行の途中状態を保存できるようにするか
...
| 実行時コンパイルのしきい値 | 実装が決めてよい。しきい値をいくつにしても結果は変わらないので、仕様では縛らない。この実装は「呼ばれた回数＋ループの回転数が 40」で、機械語にできない命令はその1つだけ仮想マシンに戻す | [runtime/execution.md](runtime/execution.md), [../docs/implementation.md](../docs/implementation.md) |
