...
- 窓（`std.ui`）は Win32 の窓が開く。キー・文字・マウス・大きさの変更・× で閉じるが届き、
  ...
  画面の細かさ（HiDPI）と広さは OS に尋ね、`ui.scale()` がそこから倍率を返す
...
面の1画素は画面の1画素にそのまま乗る。だから**寸法には `ui.scale()` を掛ける**。

```shark
var k = ui.scale();                  // 1920×1080 のふつうの画面なら 2
ui.open("さめ", 420 * k, 300 * k);   // 場面は 420×300 の升目で考える
_ = ui.font(12 * k);                 // 12 の字
```

`ui.scale()` は「描くときに掛ける倍率」で、**画面の細かさ（HiDPI）と広さ**から決まる。
掛けずに小さい面を頼むと、窓は整数倍に引き伸ばされ、描いた画素がそのまま拡大されて
字も丸も粗くなる（ゲーム機のように粗い絵を大きく出したいときは、それでよい）。
