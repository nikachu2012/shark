...
// ------------------------------------------------------------------ 同梱フォントの自動設定
//
// 内蔵ビットマップフォントは ASCII のみ対応しているため、日本語表示にはアウトラインフォントが必要となる
// （[README](../README.md) の「日本語フォントの表示」参照）。配布時の利便性を考慮し、
// 実行ファイルの近傍に配置された同梱フォントを自動検出してデフォルトとする。
//
// コア自身はファイル探索を行わない設計のため（spec/README.md 参照）、ホスト側で探索を行う。
// 検出したフォントパスを環境変数 SHARK_FONT に設定する（探索順序: SHARK_FONT → システム標準フォントパス）。
// なお、すでに SHARK_FONT が設定されている場合はユーザー指定を優先し上書きしない。
...
// ------------------------------------------------------------------ スタンドアロンバイナリ生成
// VM 実行ファイル（sharkvm）の末尾にバイトコードを結合し、16 バイトのフッターを付加する。
//
//   [ sharkvm 実行バイナリ ][ バイトコードペイロード ][ "SHARKPK1" (8B) ][ ペイロード長 (8B) ]
//
// 実行時は自プロセスの実行ファイル末尾のフッターを検出し、埋め込まれたバイトコードをデシリアライズして起動する。
// 実行ファイルのヘッダ構造を破壊しないよう、データは末尾に追記する方式を採用している。
