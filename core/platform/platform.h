...
struct PlatformScreen {
  ...
  // 半端な細かさ（1.5 など）のときは、下の pixel_ratio() が正
  int (*scale)();
  ...
  // --- 画面の細かさ（丸めないもの）。無ければ 0 -------------------------
  //
  // scale() は整数に丸めた細かさで、1 か 2 しか返らない。ところが実際には
  // **半端な細かさ**がふつうにある（Windows の 125% / 150%、ブラウザの拡大、
  // 携帯の画面）。丸めた数で面を取ると、面の1画素が画面の1画素に乗らず、
  // 機種の側で引き伸ばし直されて**にじむ**。
  //
  // ここは丸めない細かさ（1.5 など）を返す。std.ui はこれで面の大きさを
  // 決めるので、面の1画素がいつも画面の1画素に重なる（ui.pixel_ratio()）。
  // 1 より小さい数や 0 を返してはいけない。持たない機種では 0 でよく、
  // そのときは scale() の数が使われる
  double (*pixel_ratio)();
};
