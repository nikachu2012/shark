...
struct PlatformScreen {
  ...
  // --- 画面ぜんたいの大きさ（画素）。無ければ 0 -------------------------
  //
  // ui.scale()（描くときに掛ける倍率）を決めるのに使う。広い画面では
  // 1点を2画素・3画素で描いてもらったほうが、窓が切手のように小さくならず、
  // しかも引き伸ばさずに済む（spec/library/ui.md「画面の細かさ」）。
  //
  // **窓を開く前にも呼べること。**面の大きさを決める前に要る。
  // 言えない出し先は 0 でよく、そのときは細かさ（scale）だけで決まる
